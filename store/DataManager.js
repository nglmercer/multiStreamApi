...
constructor(db) {
  ...
  logger.log(`DataManager initialized for table: ${this.tableName}`);
}
...
async upsertUserFromEvent(eventData, eventType) {
  ...
  if (!userData) {
    // logger.log(`[${eventType}] Evento no contiene datos de usuario relevantes o falta userId.`);
  ...
  logger.log(`[${eventType}] Processing event for user ID: ${userId} (${userData.uniqueId || 'N/A'})`);
  ...
  try {
    ...
    if (existingUser) {
      // --- UPDATE ---
      logger.log(`[${eventType}] User ${userId} found. Updating...`);
      ...
      if (eventType === 'like' && eventData.likeCount > 0) {
        ...
        logger.log(`[like] Incrementing likes by ${eventData.likeCount} for user ${userId}`);
      }
      if (eventType === 'gift' && eventData.diamondCount > 0) {
        ...
        logger.log(`[gift] Incrementing diamonds by ${eventData.diamondCount} for user ${userId}`);
      }
      ...
      logger.log(`[${eventType}] User ${userId} updated. Changes: ${result.changes}`);

    } else {
      // --- INSERT ---
      logger.log(`[${eventType}] User ${userId} not found. Inserting...`);
      ...
      const result = await runAsync(this.db, sql, params);
      logger.log(`[${eventType}] User ${userId} inserted. New Row ID (if autoincrement): ${result.lastID}, Changes: ${result.changes}`);
    }
